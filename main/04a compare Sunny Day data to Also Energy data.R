...
dat_sunnyPortal <-
  lapply(dat_sunnyPortal,
         function(csv) read.csv(file = paste0('D:/OneDrive - hawaii.edu/Documents/Projects/HECO/Data/Raw/UH/Solar production/Sunny Portal sample data/', csv)))

# format Sunny Portal data
dat_sunnyPortal <- lapply(dat_sunnyPortal,
                          function(df) separate(df, col = colnames(df)[[1]], into = c("time","kW_sunnyPortal"), sep = ";"))
...
dat_alsoEnergy$datetime <- as.POSIXct(dat_alsoEnergy$datetime,
                                      format = "%m-%d-%Y %H:%M:%S", tz = 'HST')

# merge with Sunny Portal data
dat_combined <- left_join(dat_sunnyPortal, dat_alsoEnergy, by = 'datetime')

# plot first day
plotdat <- dat_alsoEnergy[day(dat_alsoEnergy$datetime) == 1 &
                            month(dat_alsoEnergy$datetime) == 1 &
                            year(dat_alsoEnergy$datetime) == 2020,]
ggplot(plotdat) +
...
dat_jan1 <- data.frame(value = c(dat_jan1$kWh_sunnyPortal, dat_jan1$kWh_alsoEnergy),
                       ...
                       source = rep(c('Sunny Portal', 'Also Energy'),
                                    each = nrow(dat_jan1)))
...
dat_hrly <- aggregate(list(dat_combined$kWh_sunnyPortal,
                           dat_combined$kWh_alsoEnergy),
                      list(dat_combined$hour),
                      mean)
